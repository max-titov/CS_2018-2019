...
public class Hangman extends JPanel {
	...
	public void reset(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			answerString = answerString + str.charAt(i) + " "; // a p p l e 
			currString = currString + "_ "; // _ _ _ _ _ 
		}
	}

	public boolean tryCharacter(char ch) {
		if (answerString.indexOf(ch) != -1) { // if answerString contains the char
			for (int i = 0; i < answerString.length(); i = i + 2) {
				if (answerString.charAt(i) == ch) {
					currString = currString.substring(0, i) + ch + currString.substring(i + 1);
				}
			}
			repaint();
			return true;
